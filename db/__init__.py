...
def db_connect(root=False):
    db_password = (
        os.environ.get("DB_ROOT_PASSWORD") if root else os.environ.get("DB_PASSWORD")
    )
    db_user = "root" if root else "p12admin"

    if db_password is None:
        raise ValueError(
            f"Database password environment variable not set. You are trying to login as {db_user}."
        )
